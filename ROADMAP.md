...
| Pending | v0.1.1-GP3 Headless WebAssembly Verification | Goalpost claims the full workspace unit suite runs under headless WASM, but CI only runs WASM tests for the runtime crates. | Add the missing feasible WASM coverage so every WASM-compatible workspace crate is tested under `wasm-pack test`, and document any intentionally non-WASM crates with an explicit proof. |
...
| Pending | v0.2.0-GP1 Normalized Wrappers | Roadmap calls `FixedUnitVec2` / `FixedUnitVec3` compile-time normalized wrappers, but the invariant is runtime-validated. | Implement actual compile-time proof support for known unit vectors while preserving runtime validation for dynamic inputs. |
