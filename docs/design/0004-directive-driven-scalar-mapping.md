...
2. Replace hardcoded scalar names (`BunnyScalar`, `BunnyFixedQ32_32Raw`) in
   ...
   arguments.
3. Keep the code generator extensible so that new scalar profiles can be
   introduced in the schema without editing the compiler source code.
...
scalar BunnyScalar
  ...
  @bunnyInvariant(
    description: "Bunny scalar values must be finite under the active numeric profile."
  )
```
