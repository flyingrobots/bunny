...
fn assert_scalar_bits(actual: f32, expected: f32) {
...
fn test_fixed_ray3_creation_and_conversion() {
    ...
    assert_scalar_bits(float_ray.origin.x, 1.0);
    assert_scalar_bits(float_ray.origin.y, 2.0);
    assert_scalar_bits(float_ray.origin.z, 3.0);
    assert_scalar_bits(float_ray.direction.x, 0.0);
    assert_scalar_bits(float_ray.direction.y, 1.0);
    assert_scalar_bits(float_ray.direction.z, 0.0);
...
fn test_fixed_aabb3_creation_and_conversion() {
    ...
    assert_scalar_bits(float_aabb.min.x, -1.0);
    assert_scalar_bits(float_aabb.max.z, 1.0);
...
fn test_fixed_sphere3_creation_and_conversion() {
    ...
    assert_scalar_bits(float_sphere.center.x, 5.0);
    assert_scalar_bits(float_sphere.radius, 2.5);
...
fn test_geom_validation_constructors() {
    ...
    assert_eq!(FixedAabb3::try_new(p1, p0), Err(GeomError::InvalidAabbBounds));
    ...
    assert_eq!(FixedRay3::try_new(p0, p0), Err(GeomError::InvalidRayDirection));
}
...
fn test_ray3_conversion_rejects_invalid_direction() {
    ...
    let ray = Ray3 { origin: Vec3::new(0.0, 0.0, 0.0), direction: Vec3::new(0.0, 0.0, 0.0) };
    let converted: Result<FixedRay3, GeomError> = FixedRay3::try_from(ray);

    assert_eq!(converted, Err(GeomError::InvalidRayDirection));

    let non_finite_origin =
        Ray3 { origin: Vec3::new(f32::INFINITY, 0.0, 0.0), direction: Vec3::new(1.0, 0.0, 0.0) };
    ...
    let non_finite_direction =
        Ray3 { origin: Vec3::new(0.0, 0.0, 0.0), direction: Vec3::new(f32::INFINITY, 0.0, 0.0) };
...
fn test_geom_error_implements_std_error() {
    ...
    assert_eq!(GeomError::InvalidAabbBounds.to_string(), "AABB min boundary exceeds max boundary");
    assert_eq!(GeomError::NonFiniteCoordinate.to_string(), "coordinate is not finite");
...
fn test_float_shape_conversions_reject_invalid_bounds() {
    let invalid_aabb = Aabb3 { min: Vec3::new(1.0, 0.0, 0.0), max: Vec3::new(0.0, 0.0, 0.0) };
    ...
    let sub_lsb_invalid_aabb =
        Aabb3 { min: Vec3::new(1e-12, 0.0, 0.0), max: Vec3::new(0.0, 0.0, 0.0) };
    ...
    let non_finite_aabb =
        Aabb3 { min: Vec3::new(f32::NAN, 0.0, 0.0), max: Vec3::new(0.0, 0.0, 0.0) };
    ...
    let sub_lsb_invalid_sphere = Sphere3 { center: Vec3::new(0.0, 0.0, 0.0), radius: -1e-12 };
    let converted_sub_lsb_sphere: Result<FixedSphere3, GeomError> =
        FixedSphere3::try_from(sub_lsb_invalid_sphere);
    assert_eq!(converted_sub_lsb_sphere, Err(GeomError::NegativeSphereRadius));

    let non_finite_sphere = Sphere3 { center: Vec3::new(0.0, 0.0, 0.0), radius: f32::INFINITY };
    ...
    let non_finite_center_sphere =
        Sphere3 { center: Vec3::new(f32::INFINITY, 0.0, 0.0), radius: 1.0 };
...
fn test_explicit_float_boundary_conversion_apis() {
    ...
    let fixed_sphere = sphere.try_into_fixed().expect("valid sphere should convert");
