...
fn seeded_axis_corpus(seed: u64) -> SeededAxisCorpus {
    let mut state = seed;
    let sphere_center_z = 4 + bounded_offset(&mut state, 3);
    let aabb_min_z = 2 + bounded_offset(&mut state, 3);
    let aabb_max_z = aabb_min_z + 2 + bounded_offset(&mut state, 2);
    let triangle_z = 3 + bounded_offset(&mut state, 4);
...
fn ray_query_raw_output_determinism_corpus() {
    ...
    let seeded_sphere = FixedSphere3::new(vec3(0, 0, seeded.sphere_center_z), q32(1));
    assert_sphere_raw(
        &unit_z_ray,
        &seeded_sphere,
        Some(([0, 0, raw(seeded.sphere_center_z - 1)], [0, 0, -ONE_RAW])),
    );

    let seeded_aabb = FixedAabb3::new(
        vec3(-1, -1, seeded.aabb_min_z),
        vec3(1, 1, seeded.aabb_max_z),
    );
    assert_aabb_raw(
        &unit_z_ray,
        &seeded_aabb,
        Some((raw(seeded.aabb_min_z), raw(seeded.aabb_max_z))),
    );

    assert_triangle_raw(
        &unit_z_ray,
        vec3(-1, -1, seeded.triangle_z),
        vec3(1, -1, seeded.triangle_z),
        vec3(0, 1, seeded.triangle_z),
        Some([0, 0, raw(seeded.triangle_z)]),
    );
}
